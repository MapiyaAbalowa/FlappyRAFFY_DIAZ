...
# Flappy Raffy MEME in Python PyGame #

# A GAME PROJECT PRESENTED TO: Univeristy of the Philippines High School in Iloilo
# Clone GitHub repository: "bash:~ user$ git-clone https://github.com/MapiyaAbalowa/Flappy-Bird-V2" (for macOS Terminal) #
...
# The Sprites and the Background picture being utilised in this game were taken from Google images...#
...
def multimedia():
    ...
    zir = pygame.image.load("zir.jpg").convert()
    zir = pygame.transform.scale(zir, [1220, 730])
    ...
    raffy = pygame.transform.scale(pygame.image.load("raffy.jpeg").convert_alpha(), (50, 40))
    ...
    diyas = pygame.transform.scale(pygame.image.load("diyas.jpg").convert_alpha(), (98, 500))
    noel = pygame.transform.scale(pygame.image.load("noel.jpeg").convert_alpha(), (98, 500))

    scoreboard = pygame.image.load("scoreboard.png").convert_alpha()
    milez = pygame.image.load("milez.jpeg").convert_alpha()
    milez = pygame.transform.scale(milez, [150, 150])

    sprites = [raffy, "b1", "b2", "b3", noel, diyas, scoreboard, milez, zir]
...
class Main_Platform(object):
    ...
    def __init__(game):
        ...
        pygame.display.set_caption("Flappy Zir Raffy")
